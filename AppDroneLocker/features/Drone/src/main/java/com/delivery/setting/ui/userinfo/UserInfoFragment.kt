...
//            viewModel.handleEvent(UserInfoEvent.SaveUserInfo(name, phoneNumber))
            ...
            appNavigation.openTabAccountToLoginScreen()
        }
    }
    ...
    private fun handleViewState(state: UserInfoViewState) {
        binding.apply {
            ...
            // Set locker data
            if (state.lockerName.isNotEmpty()) {
                tvLockerName.text = state.lockerName
            } else {
                tvLockerName.text = "Chưa chọn locker"
            }

            if (state.lockerId.isNotEmpty()) {
                tvLockerId.text = state.lockerId
            } else {
                tvLockerId.text = "-"
            }
