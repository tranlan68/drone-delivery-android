package com.delivery.setting.utils
                ...
                mainLockerDto.lockers?.forEach { nestedLockerDto ->
                    val nestedLocker =
                        mapToLocker(
                            nestedLockerDto,
                            isMainLocker = false,
                            parentLockerName = mainLockerDto.name,
                        )
                    allLockers.add(nestedLocker)
                    Timber.tag("LockerDisplayHandler")
                        .d("Added nested locker: ${nestedLocker.lockerName} (parent: ${mainLockerDto.name})")
                }
            }

            Timber.tag("LockerDisplayHandler")
                .d("Total lockers flattened: ${allLockers.size} (from ${lockerDtos.size} main lockers)")
            return allLockers
        }
