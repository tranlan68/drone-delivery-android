...
import com.delivery.setting.utils.LockerDisplayHandler
...
class LockerRepository
    ...
    constructor(
         ...
         */
        suspend fun getLockers(): Flow<List<Locker>> =
            flow {
                try {
         ...
         */
        suspend fun getMainLockersOnly(): Flow<List<Locker>> =
            flow {
                try {
                    if (cachedLockers == null) {
                        Timber.d("Fetching lockers from API")
         ...
         */
        suspend fun getNestedLockersOnly(): Flow<List<Locker>> =
            flow {
                try {
                    if (cachedLockers == null) {
                        Timber.d("Fetching lockers from API")
         ...
         */
        private suspend fun getLockerDtos(): Flow<List<LockerDto>> =
            flow {
                try {
                    if (cachedLockers == null) {
                        Timber.d("Fetching lockers from API")
        ...
        suspend fun getLockerPositionById(lockerId: String): org.maplibre.android.geometry.LatLng? {
            return try {
                ...
                Timber.e(e, "Error getting locker position for ID: $lockerId")
                return null
            }
        }
