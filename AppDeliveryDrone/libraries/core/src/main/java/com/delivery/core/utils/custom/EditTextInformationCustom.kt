...
class EditTextInformationCustom : ConstraintLayout {
    ...
    private var onClickEditText: (() -> Unit)? = null
...
//        }
        ...
        binding.edtInformation.setOnClickListener {
            onClickEditText?.invoke()
        }
    }
    ...
    fun hideFocusable() {
        binding.edtInformation.apply {
            isFocusable = false
            isCursorVisible = false
            isClickable = true // để nhận sự kiện click
            inputType = InputType.TYPE_NULL // ngăn bàn phím hiện lên
        }
    }

    fun setOnClickEditText(onClick: (() -> Unit)?) {
        this.onClickEditText = onClick
    }
